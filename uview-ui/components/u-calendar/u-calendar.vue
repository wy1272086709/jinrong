	 ...
	 :safeAreaInsetBottom="safeAreaInsetBottom" @close="close" :z-index="uZIndex" :border-radius="borderRadius" :closeable="closeable">
		<view class="u-calendar">
			...
			<view class="u-calendar__action u-flex u-row-center">
				...
				<picker mode="date" :end="endYear+'-'+endMonth" :value="monthVal" fields="month" @change="bindDateChange">
					<!--
	...
	export default {
		...
		methods: {
			bindDateChange(e) {
				...
				console.log(e)
				this.year = e.detail.value.substr(0, 4)
				const m = e.detail.value.substr(5, 2);
				this.month= m.substr(0,1) == '0'? m.substr(1): m
			...
			init() {
				...
				this.endDate = "";
				this.isStart = true;
				this.monthVal = `${now.getFullYear()}-${now.getMonth() + 1}`
				this.changeData();
			},
			//日期处理
			initDate(date) {
