<template>
	<view class="easy-select" @click.stop="trigger" :style="[easySelectSize]">
		<!-- Boolean 属性,默认都用false，这样传递这个值，就用true -->
		...
		-->
		<u-icon name="arrow-up" :custom-style="{position: 'absolute',
    right: '2px',
    'align-items': 'center',
    display: 'flex', height: '54rpx'}" size="28" color="#999" v-if="showSuffix == 'showOptions'"></u-icon>
		
		<u-icon name="arrow-down" :custom-style="{position: 'absolute',
    right: '2px',
    'align-items': 'center',
    display: 'flex', height: '54rpx'}" size="28" color="#999" v-if="showSuffix == 'no-showOptions'"></u-icon>
